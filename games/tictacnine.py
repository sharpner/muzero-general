...
class MuZeroConfig:
    def __init__(self):
        ...
        active_configuration = configuration_4

        self.seed = 0  # Seed for numpy, torch and the game
        self.max_num_gpus = None  # Fix the maximum number of GPUs to use. It's usually faster to use a single GPU (set it to 1) if it has enough memory. None will use every GPUs available

        ### Game
        self.observation_shape = (3, 81,1)  # Dimensions of the game observation, must be 3D (channel, height, width). For a 1D array, please reshape it to (1, 1, length of array)
        ...
        ### Replay Buffer
        self.replay_buffer_size = 6000 #3000  # Number of self-play games to keep in the replay buffer
        self.num_unroll_steps = 30 #20  # Number of game moves to keep for every batch element
        self.td_steps = 40 #20  # Number of steps in the future to take into account for calculating the target value
...
class TicTacNine:
    def __init__(self):
        self.board = numpy.zeros((81, 1), dtype="int32")
        self.player = 1
        self.last_move = None
    ...
    def get_observation(self):
        ...
        board_to_play = numpy.full((81, 1), self.player)
    ...
    def expert_action(self):
        ...
        for legal_action in self.legal_actions():
            ...
            # (1) a bit of randomness for each move, just in case
            # they fulfill the same criteria
            thisScore =random.randint(5,14)

            forced_field = int(legal_action % 9)
            target_field = board[forced_field:forced_field+9]

            # (2) if target field results in player being able to chose
            # deduct points
            ...
            # (4) a good strategy seems to be to play always to stay
    ...
    def have_winner(self):
        return is_won(self.board) == self.player


    def render(self):
        for i in range(3):
            m = i * 3
            print(f'{pc(self.board[m])}', end='')
            print(f'{pc(self.board[1+m])}', end='')
            print(f'{pc(self.board[2+m])}', end='')
            print("|", end='')

            print(f'{pc(self.board[9+m])}', end='')
            print(f'{pc(self.board[10+m])}', end='')
            print(f'{pc(self.board[11+m])}', end='')

            print("|", end='')

            print(f'{pc(self.board[18+m])}', end='')
            print(f'{pc(self.board[19+m])}', end='')
            print(f'{pc(self.board[20+m])}', end='')
            print("|\n", end='')
        ...
        for i in range(3):
            m = i * 3
            print(f'{pc(self.board[27+m])}', end='')
            print(f'{pc(self.board[28+m])}', end='')
            print(f'{pc(self.board[29+m])}', end='')
            print("|", end='')

            print(f'{pc(self.board[36+m])}', end='')
            print(f'{pc(self.board[37+m])}', end='')
            print(f'{pc(self.board[38+m])}', end='')
            print("|", end='')

            print(f'{pc(self.board[45+m])}', end='')
            print(f'{pc(self.board[46+m])}', end='')
            print(f'{pc(self.board[47+m])}', end='')
            print("|\n", end='')
        ...
        for i in range(3):
            m = i * 3
            print(f'{pc(self.board[54+m])}', end='')
            print(f'{pc(self.board[55+m])}', end='')
            print(f'{pc(self.board[56+m])}', end='')
            print("|", end='')

            print(f'{pc(self.board[63+m])}', end='')
            print(f'{pc(self.board[64+m])}', end='')
            print(f'{pc(self.board[65+m])}', end='')
            print("|", end='')

            print(f'{pc(self.board[72+m])}', end='')
            print(f'{pc(self.board[73+m])}', end='')
            print(f'{pc(self.board[74+m])}', end='')
            print("|\n", end='')
